...
## 작업 진행 방식 (plan/ 사이클 파일 연동, 사람 파트너 검토 포함)

각 사이클마다 사람 파트너의 검토를 받는 세 단계로 진행한다. 단계를 건너뛰거나 검토 요청 없이 다음 단계로 넘어가지 말 것. 계획은 `PLAN.md` 인덱스가 가리키는 `plan/` 디렉터리의 사이클별 파일에 작성한다.

### RED 단계
- `plan/` 디렉터리에 이번 사이클 파일을 작성(또는 이미 있는 파일을 확인)해 달성할 목표(구현할 동작, 작성할 실패 테스트)를 적고, `PLAN.md` 인덱스에 항목을 추가한다.
- 실패하는 테스트를 작성하고, 실제로 실패하는 것을 직접 확인한다.
- 작성한 목표와 실패 테스트를 **사람 파트너에게 검토 요청**한다.

### GREEN 단계
- RED 단계에서 사이클 파일에 적힌 목표를 달성하도록 구현한다.
- **오버 엔지니어링 금지** — 테스트를 통과시키는 데 필요한 최소한의 코드만 작성한다.
- 테스트가 실제로 통과하는지 직접 확인한다.
- 구현 결과를 **사람 파트너에게 검토 요청**한다.

### REVIEW 단계
- GREEN 단계에서 작성된 코드를 리뷰한다.
- 사이클 파일에 명시된 범위 외의 것이 구현되지 않았는지 확인한다.
- 리팩터링이 필요하다고 판단되면, 리팩터링 전에 **사람 파트너에게 검토 요청**한다.

이 세 단계가 끝나면 다음 기능/동작에 대해 RED 단계부터 다시 시작한다.
