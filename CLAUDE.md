...
- **RED → GREEN → REVIEW 사이클**: 각 기능 단위마다 `plan/` 디렉터리의 사이클별 파일(목록은 `PLAN.md` 인덱스 참고)에 목표를 적고 검토를 받은 뒤(RED) 최소 구현으로 테스트를 통과시키고 검토를 받으며(GREEN), 구현이 해당 사이클 범위를 벗어나지 않았는지 확인하고 필요 시 리팩터링 전 검토를 받는다(REVIEW).
