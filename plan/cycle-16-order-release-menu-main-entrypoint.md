...
# Cycle 16 — 출고 처리 메뉴 + `main.py` 진입점 (프로젝트의 마지막 사이클)
...
위 목표/범위(특히 설계 판단 2번 — `list_pending_orders()`/`list_releasable_orders()`를
`OrderController`에 추가하는 판단, 설계 판단 4번 — `summary` 집계를 `main.py`의 순수 함수
`build_summary()`로 두는 판단, 설계 판단 5번 — 저장소/레지스트리 4개 인스턴스를 프로세스
전체에서 단 하나씩만 공유하도록 조립하는 판단, 설계 판단 6번 — `run_main_loop()`을 분리해
`input`/`capsys`/`tmp_path` 기반 통합 테스트로 검증하는 전략, 설계 판단 7번 — 손상된 시료
파일 안내를 계속 범위 밖으로 두는 판단, 설계 판단 8번 — 사이클을 분할하지 않고 로드맵대로
마지막 사이클로 마무리하는 판단)으로 RED 단계를 진행해도 될지 검토 부탁드립니다.
