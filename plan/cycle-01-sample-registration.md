...
`test_수율이_음수이면_등록을_거부한다`)를 추가했다. 이어서 verify-agent가 지적한 커버리지
갭(`yield_rate == 1.0`이 실제로 허용되는지 검증하는 테스트 부재)을 메우기 위해
`test_수율이_1이면_등록을_허용한다` 테스트를 추가했다(`690e4e0`, 기존 구현이 이미 올바르게
동작함을 확인하는 커버리지 보강이라 별도 GREEN 수정은 없었다).
