...
# Cycle 12 — 주문/생산 컨트롤러 영속화 연동 (`OrderController`/`ProductionController` ↔ Repository)
...
**다음 사이클**: 아직 계획되지 않음 (예정: Cycle 13 — 콘솔 View 골격 + 메인 메뉴 + 시료 관리 메뉴)
...
위 목표/범위(특히 설계 판단 1~4번 — `OrderRegistry.restore()` 신설, 저장소 필수 주입,
메서드별 저장 대상 표, `OrderController.list_orders()` 추가 여부)로 RED 단계를 진행해도 될지
검토 부탁드립니다.
