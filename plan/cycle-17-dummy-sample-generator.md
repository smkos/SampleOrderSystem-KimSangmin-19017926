...
# Cycle 17 — 더미 시료(Sample) 데이터 생성기 (`DummyDataGenerator_PoC` 이식, 로드맵 확장)
...
위 목표/범위(특히 설계 판단 1번 — 순수 함수만 이번 사이클에 다루고 저장 wrapper는 다음
사이클로 미루는 판단, 설계 판단 2번 — 새 최상위 `devtools/` 디렉터리 신설 판단, 설계 판단 3번 —
`SampleRegistry`가 채번하지 않는 `sample_id`를 생성기가 직접 채번하는 판단, 설계 판단 4번 —
PoC의 `dict` 대신 `Sample` 객체를 직접 반환하는 판단, 설계 판단 6번 — 더미 `Order` 생성/wrapper/
콘솔 메뉴 연동을 별도 사이클로 미루는 판단)으로 RED 단계를 진행해도 될지 검토 부탁드립니다.
