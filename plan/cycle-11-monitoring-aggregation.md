...
- `controller/monitoring_controller.py` (신규):
  ...
  - `MonitoringController.stock_status_by_sample() -> dict[str, str]`: `sample_registry.
    ...
    반환. (**갱신**: 이 반환 타입은 이후 Cycle 15 "추가 보완"에서
    `dict[str, dict]`(`{sample_id: {"label": str, "stock_qty": int}}`)로 바뀌었다 —
    `plan/cycle-15-console-view-monitoring-production-menus.md`의 "추가 보완" 절 참고.)
