...
**다음 사이클**: [Cycle 17 — 더미 시료 데이터 생성기](cycle-17-dummy-sample-generator.md)
(`PRD.md`/`SPEC.md` 기능 명세 기준으로는 이 Cycle 16이 마지막이었으나, 사람 파트너의 요청으로
로드맵이 확장되어 Cycle 17이 이어졌다.)
...
사람 파트너가 위 수동 실행 검증 과정에서 `python main.py`를 직접 실행하다가 버그를
발견했다 — 시료 등록·주문 생성 등에서 숫자를 기대하는 입력에 숫자가 아닌 값을 입력하면
`ValueError`가 처리되지 않고 그대로 전파되어 프로그램이 죽는 문제였다. 이를 커밋
`e5a8069`(RED)/`628a265`(GREEN)로 보완했다 — `ConsoleView.show_error()`를 추가하고,
`main.py`의 각 하위 메뉴(시료 등록, 시료 주문 등)에서 `ValueError`를 잡아 안내 메시지를
출력한 뒤 프로그램이 죽지 않고 계속 진행하도록 `try/except` 처리를 추가했다. 신규 테스트
3개(`test_에러_메시지를_출력한다`, `test_존재하지_않는_시료ID로_주문하면_에러_메시지를_출력하고_계속_진행한다`,
`test_시료_등록에서_수량에_숫자가_아닌_값을_입력해도_프로그램이_죽지_않는다`)를 추가했으며,
전체 테스트 142개가 회귀 없이 통과한다.
