...
## 작업 진행 방식 (PLAN.md 연동, 사람 파트너 검토 포함)

각 사이클마다 사람 파트너의 검토를 받는 세 단계로 진행한다. 단계를 건너뛰거나 검토 요청 없이 다음 단계로 넘어가지 말 것.

### RED 단계
- `PLAN.md`에 이번 사이클에서 달성할 목표(구현할 동작, 작성할 실패 테스트)를 작성한다.
- 실패하는 테스트를 작성하고, 실제로 실패하는 것을 직접 확인한다.
- 작성한 목표와 실패 테스트를 **사람 파트너에게 검토 요청**한다.

### GREEN 단계
- RED 단계에서 `PLAN.md`에 적힌 목표를 달성하도록 구현한다.
...
- `PLAN.md`에 명시된 범위 외의 것이 구현되지 않았는지 확인한다.
